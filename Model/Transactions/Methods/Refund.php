...
class Refund extends Method
{
    ...
    private function doUpdates($config)
    {
        try {
            ...
            if ($config->value) {
                ...
                $this->notifyCustomer($config->order_id, $config->welight_status, $comment);
            } else {
                $this->addStatusToOrder($config->order_id, 'welight_devolvida');
    ...
    private function requestRefund($config)
    {
        ...
        try {
            return \welight\Services\Transactions\Refund::create(
    ...
    /**
     * Updates respective order partially refunded status to 1 in welight_orders table
     *
    ...
    {
        $this->updatePartiallyRefundedwelight($orderId);
    }
