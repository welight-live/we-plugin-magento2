...
class Abandoned extends Method
{
    ...
    public function request()
    {
        ...
        $this->getwelightAbandoned();

        if ($this->_welightPaymentList->getTransactions()) {
            foreach ($this->_welightPaymentList->getTransactions() as $payment) {
                date_default_timezone_set('UTC');
                $order = \UOL\welight\Helper\Data::getReferenceDecryptOrderID($payment->getReference());
                $order = $this->_order->load($order);
                if ($this->getStoreReference() == \UOL\welight\Helper\Data::getReferenceDecrypt(
     ...
     * @throws \Exception
     */
    private function getwelightAbandoned($page = null)
    {
        ...
        try {
            ...
            if (is_null($this->_welightPaymentList)) {
                $this->_welightPaymentList = $this->requestwelightAbandoned($page);
            } else {

                $response = $this->requestwelightAbandoned($page);
    ...
    private function sendEmail($order, $recoveryCode)
    {
        ...
        $transport = $this->_transportBuilder->setTemplateIdentifier('abandoned_template')
            ...
            ->setTemplateVars([
                'order' => $order,
                'welight_recover_url' => $this->abandonedRecoveryUrl($recoveryCode)
            ])
            ->setFrom($sender)
            ->addTo($receiver)
            ->getTransport();
     ...
     * @throws \Exception
     */
    protected function requestwelightAbandoned($page)
    {
        ...
        try {
            ...
            return \welight\Services\Transactions\Search\Abandoned::search(
    ...
    /**
     * Build a URL for recovery a welightTransaction
     *
    ...
    private function abandonedRecoveryUrl($recoveryCode)
    {
        ...
        if (strtolower($this->_library->getEnvironment()) == "sandbox") {
            return 'https://sandbox.welight.uol.com.br/checkout/v2/resume.html?r=' . $recoveryCode;
        }
        return 'https://welight.uol.com.br/checkout/v2/resume.html?r=' . $recoveryCode;
    }
    ...
    private function getSent($orderId)
    {
        ...
        $tableName = $this->_resource->getTableName('welight_orders');
    ...
    /**
     * Increments a sent for a order in welight_orders table.
     *
     * @param $orderId
     * @param $sent
     */
