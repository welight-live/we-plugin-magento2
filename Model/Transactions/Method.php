...
namespace UOL\welight\Model\Transactions;
...
abstract class Method
{
    ...
    /**
     * Get welight payments
     *
     * @param null $page
     * @return \welight\Parsers\Transaction\Search\Date\Response|string
     * @throws \Exception
     */
    protected function getTransactions($page = null)
    {
        ...
        try {
            ...
            if (is_null($this->_welightPaymentList)) {
                $this->_welightPaymentList = $this->requestwelightPayments($page);
            } else {
                $response = $this->requestwelightPayments($page);
                //update some important data
                $this->_welightPaymentList->setDate($response->getDate());
                $this->_welightPaymentList->setCurrentPage($response->getCurrentPage());
                $this->_welightPaymentList->setResultsInThisPage(
                    $response->getResultsInThisPage() + $this->_welightPaymentList->getResultsInThisPage()
                );
                //add new transactions
                $this->_welightPaymentList->addTransactions($response->getTransactions());
            }

            //check if was more pages
            if ($this->_welightPaymentList->getTotalPages() > $page) {
                $this->getwelightPayments(++$page);
            }
        } catch (\Exception $exception) {
            throw $exception;
        }
        return $this->_welightPaymentList;
    }

    /**
     * Get all transactions where there is a welight transaction code
     * @return array
     * @throws \Exception
     */
    public function searchTransactions()
    {
        try {
            ...
            $select = $connection->select()
                ...
                ->join( ['ps' => 'welight_orders'], 'order.entity_id = ps.order_id')
            ...
            if ($this->_scopeConfig->getValue('payment/welight/environment')) {
                $select = $select->where('ps.environment = ?', $this->_scopeConfig->getValue('payment/welight/environment'));
            }
            ...
            if (!empty($this->_idwelight)) {
                $select = $select->where('ps.transaction_code = ?', $this->_idwelight);
            }
    ...
    /**
     * Request welightTransaction details by code
     * @param $code
     *
     * @return null|object
     * @throws Exception
    ...
    public function getTransactionsByCode($code)
    {
        ...
        $response = null;
        try {
            $response = \welight\Services\Transactions\Search\Code::search(
    ...
    /**
     * Request all welightTransaction in this _date interval
     *
     * @param $page
     * @return string
     * @throws \Exception
     */
    protected function requestwelightPayments($page)
    {
        ...
        try {
            ...
            return \welight\Services\Transactions\Search\Date::search(
                $this->_library->getwelightCredentials(),
                $options
            );
    ...
    {
        return $this->_order->load(\UOL\welight\Helper\Data::getReferenceDecryptOrderID($payment->getReference()));
    }
    ...
    {
        return \UOL\welight\Helper\Data::getStatusFromKey($status);
    }
    ...
    {
        return \UOL\welight\Helper\Data::getPaymentStatusToString($status);
    }
    ...
    protected function getKeyFromOrderStatus($status)
    {
        ...
        return \UOL\welight\Helper\Data::getKeyFromStatus($param);
    }
     ...
     * @return bool|string
     */
    protected function formatwelightStatus($payment)
    {
    ...
    {
        return \UOL\welight\Helper\Data::getReferenceDecrypt($payment->getReference());
    }
    ...
    {
        return $this->_scopeConfig->getValue('welight/store/reference');
    }
    ...
    /**
     * Update the `welight_orders` table
     *
     * @param int $orderId
     * @param string $transactionCode
     * @return void
     */
    protected function updatewelightOrders($orderId, $transactionCode)
    {
        $this->updateOrders('welight_orders', $orderId, $transactionCode);
    }
    ...
    /**
     * Update column 'partially_refunded' the `welight_orders` table
     *
     * @param int $orderId
     * @return void
     */
    protected function updatePartiallyRefundedwelight($orderId)
    {
        $this->getConnection()
            ->query(sprintf(
                "UPDATE `%s` SET partially_refunded = 1 WHERE entity_id='%s'",
                $this->getPrefixTableName('welight_orders'),
                $orderId
            ));
    }

    /**
     * Get all welight partially refunded orders id
     *
     * @return array
     */
    protected function getPartiallyRefundedOrders()
    {
        $welightOrdersIdArray = array();

        $connection = $this->getConnection();
        $select = $connection->select()
            ->from( ['ps' => $this->getPrefixTableName('welight_orders')], ['order_id'] )
        ...
        foreach ($connection->fetchAll($select) as $value) {
            $welightOrdersIdArray[] = $value['order_id'];
        }

        return $welightOrdersIdArray;
    }
    ...
    public function buildDetailsTransaction()
    {
        return array(
            ...
            'type'              => \UOL\welight\Helper\Data::getTransactionTypeName($this->_detailsTransactionByCode->getType()),
            'status'            => \UOL\welight\Helper\Data::getPaymentStatusToString($this->_detailsTransactionByCode->getStatus()),
            ...
            'cancelationSource' => \UOL\welight\Helper\Data::getTitleCancellationSourceTransaction($this->_detailsTransactionByCode->getCancelationSource()),
    ...
    private function preparePaymentMethod()
    {
        ...
        if(!empty($this->_detailsTransactionByCode->getPaymentMethod()))
        {
            $paymentMethod = array(
                ...
                'titleType' => \UOL\welight\Helper\Data::getTitleTypePaymentMethod($this->_detailsTransactionByCode->getPaymentMethod()->getType()),
                'titleCode' => \UOL\welight\Helper\Data::getTitleCodePaymentMethod($this->_detailsTransactionByCode->getPaymentMethod()->getCode())
            );
        }
