...
use welight\Domains\Requests\Payment as PS_Payment;
...
class PaymentMethod
{
    ...
    /**
     *
     * @var \welight\Domains\Requests\Payment
     */
    ...
    public function createPaymentRequest()
    {
        ...
        // welight Payment Method discounts
        $this->setwelightDiscountsByPaymentMethod();
        //Shipping
        ...
        } catch (welightServiceException $ex) {
    ...
    private function setItemsInformation()
    {
        foreach ($this->_checkoutSession->getLastRealOrder()->getAllVisibleItems() as $product) {
            $this->_paymentRequest->addItems()->withParameters(
                ...
                \UOL\welight\Helper\Data::fixStringLength($product->getName(), 255), //description
                $product->getSimpleQtyToShip(), //quantity
                \UOL\welight\Helper\Data::toFloat($product->getPrice()), //amount
            ...
            if ($shipping) {
                if (count($shipping->getStreet()) === 4) {
                    $this->_paymentRequest->setShipping()->setAddress()->withParameters(
                        ...
                        \UOL\welight\Helper\Data::fixPostalCode($shipping->getPostcode()),
                        $shipping->getCity(),
                    ...
                    $address = \UOL\welight\Helper\Data::addressConfig($shipping['street']);
                ...
                $this->_paymentRequest->setShipping()->setType()
                    ->withParameters(\welight\Enum\Shipping\Type::NOT_SPECIFIED); //Shipping Type
    ...
    private function getShippingAddress($address, $shipping = null)
    {
        ...
        if ($shipping) {
            return \UOL\welight\Helper\Data::addressConfig($shipping['street']);
        }
        return null;
    }
    ...
    {
        $connectionData = new \welight\Resources\Connection\Data($this->_library->getwelightCredentials());
    ...
    {
        return \UOL\welight\Helper\Data::getOrderStoreReference(
            $this->_scopeConfig->getValue('welight/store/reference'),
    ...
    private function getRegionAbbreviation($shipping)
    {
        ...
        $regionAbbreviation = new \welight\Enum\Address();
    ...
    {
        return $this->_scopeConfig->getValue('payment/welight/notification');
    }
    ...
    {
        return $this->_scopeConfig->getValue('payment/welight/redirect');
    }
    ...
    private function setSenderPhone()
    {
        ...
        if (! empty($addressData['telephone'])) {
            $phone = \UOL\welight\Helper\Data::formatPhone($addressData['telephone']);
    ...
    /**
     * Set welight recovery shopping cart value
     *
     * @return void
     */
    private function setShoppingCartRecovery()
    {
        if ($this->_scopeConfig->getValue('payment/welight/shopping_cart_recovery') == true) {
    ...
    /**
     * Get the discount configuration for welight store configurarion and
     * set in the payment request the discount amount for every payment method configured
     *
     * @return void
     */
    private function setwelightDiscountsByPaymentMethod()
    {
        $storeId = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        if ($this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_credit_card', $storeId) == 1) {
            $creditCard = (double)$this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_credit_card_value', $storeId);
            if ($creditCard && $creditCard != 0.00) {
                $this->_paymentRequest->addPaymentMethod()->withParameters(
                    \welight\Enum\PaymentMethod\Group::CREDIT_CARD,
                    \welight\Enum\PaymentMethod\Config\Keys::DISCOUNT_PERCENT,
                    $creditCard
                );
            }
        }
        if ($this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_online_debit', $storeId) == 1) {
            $eft = (double)$this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_online_debit_value', $storeId);
            if ($eft && $eft != 0.00) {
                $this->_paymentRequest->addPaymentMethod()->withParameters(
                    \welight\Enum\PaymentMethod\Group::EFT,
                    \welight\Enum\PaymentMethod\Config\Keys::DISCOUNT_PERCENT,
                    $eft
                );
            }
        }
        if ($this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_boleto', $storeId) == 1) {
            $boleto = (double)$this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_boleto_value', $storeId);
            if ($boleto && $boleto != 0.00) {
                $this->_paymentRequest->addPaymentMethod()->withParameters(
                    \welight\Enum\PaymentMethod\Group::BOLETO,
                    \welight\Enum\PaymentMethod\Config\Keys::DISCOUNT_PERCENT,
                    $boleto
                );
            }
        }
        if ($this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_deposit_account', $storeId)) {
            $deposit = (double)$this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_deposit_account_value', $storeId);
            if ($deposit && $deposit != 0.00) {
                $this->_paymentRequest->addPaymentMethod()->withParameters(
                    \welight\Enum\PaymentMethod\Group::DEPOSIT,
                    \welight\Enum\PaymentMethod\Config\Keys::DISCOUNT_PERCENT,
                    $deposit
                );
            }
        }
        if ($this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_balance', $storeId)) {
            $balance = (double)$this->_scopeConfig->getValue('payment/welight_default_lightbox/discount_balance_value', $storeId);
            if ($balance && $balance != 0.00) {
                $this->_paymentRequest->addPaymentMethod()->withParameters(
                    \welight\Enum\PaymentMethod\Group::BALANCE,
                    \welight\Enum\PaymentMethod\Config\Keys::DISCOUNT_PERCENT,
                    $balance
                );
            }
