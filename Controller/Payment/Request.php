...
use UOL\welight\Model\Direct\BoletoMethod;
use UOL\welight\Model\Direct\CreditCardMethod;
use UOL\welight\Model\Direct\DebitMethod;
...
class Request extends \Magento\Framework\App\Action\Action
{
    ...
    public function __construct(
        ...
        /** @var \UOL\welight\Model\PaymentMethod payment */
    ...
    public function execute()
    {
        ...
        if ($paymentData['method'] === 'welight_boleto') {
            try {
                ...
                if (! isset($paymentData['additional_information']['boleto_document'])
                    ...
                    throw new \Exception("Error passing data from checkout page to welight Request Controller");
                }

                $this->order = $this->loadOrder($this->orderId);
                /** @var \UOL\welight\Model\Direct\BoletoMethod $boleto */
                ...
                return $this->_redirect(sprintf('%s%s', $this->baseUrl(), 'welight/direct/success'));
            } catch (\Exception $exception) {
                if (!is_null($this->order)) {
                    $this->changeOrderHistory('welight_cancelada');
                }
                $this->welightLogger($exception->getMessage());
                $this->clearSession();
                $this->whenError($exception->getMessage());
                return $this->_redirect(sprintf('%s%s', $this->baseUrl(), 'welight/payment/failure'));
            }
        }

        if ($paymentData['method'] === 'welight_online_debit') {
            try {
                $this->orderId = $lastRealOrder->getId();
                ...
                $this->order = $this->loadOrder($this->orderId);
                /** @var \UOL\welight\Model\Direct\DebitMethod $debit */
         ...
         if ($paymentData['method'] === 'welight_credit_card') {
            try {
                $this->orderId = $lastRealOrder->getId();
                ...
                $this->order = $this->loadOrder($this->orderId);
                /** @var \UOL\welight\Model\Direct\CreditCardMethod $creditCard */
    ...
    {
        $this->changeOrderHistory('welight_aguardando_pagamento');
    ...
    private function whenSuccess($response, $method)
    {
        ...
        return $this->result->setData([
            'success' => true,
            'payload' => [
                'redirect' => sprintf('%s%s', $this->baseUrl(), 'welight/direct/success')
            ]
        ]);
    }
    
    /**
     * Create new welight payment session data
     *
     * @param $response
     */
    private function makeSession($response, $method)
    {
        if ($method === 'welight_credit_card') {
            ...
            $this->session()->setData([
                'welight_payment' => ['payment_link' => $response->getPaymentLink(),
                    'payment_type'  => $method,
                    'order_id'      => $this->orderId,
                ]
            ]);
     ...
     */
    private function welightLogger($errorMessage)
    {
        $logger = $this->_objectManager->create('\Psr\Log\LoggerInterface');
        $logger->addError('[Uol welight Module][Payment Error] ' .$errorMessage);
    }
}
