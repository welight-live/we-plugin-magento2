...
class Checkout extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $code = $result->getCode();
        $resultPage->getLayout()->getBlock('welight.payment.checkout')
            ->setCode($code);
        $resultPage->getLayout()->getBlock('welight.payment.checkout')
            ->setPaymentJs($this->getwelightPaymentJs());
        $resultPage->getLayout()->getBlock('welight.payment.checkout')
     ...
     * @throws \Exception
     */
    private function getwelightPaymentJs()
    {
        if (\welight\Configuration\Configure::getEnvironment()->getEnvironment() == 'sandbox') {
            return \UOL\welight\Helper\Library::SANDBOX_JS;
        } else {
            return \UOL\welight\Helper\Library::STANDARD_JS;
        }
    }
}
