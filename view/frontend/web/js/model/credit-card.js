...
//  return welightDirectPayment.setSessionId(session)
//}
//
//function getSenderHash() {
//  return welightDirectPayment.getSenderHash()
//}
//
...
function getBrand(self) {
  ...
  welightDirectPayment.setSessionId(document.getElementById('welightCcSessionId').value);
  if (validateCreditCard(self)) {
    welightDirectPayment.getBrand({
      cardBin: unmask(document.getElementById('welight_credit_card_number').value),
      success: function (response) {
        document.getElementById('creditCardBrand').value = response.brand.name
        getInstallments(response.brand.name)
        displayError(document.getElementById('welight_credit_card_number'), false)
      },
      error: function () {
    ...
    displayError(document.getElementById('welight_credit_card_number'))
  }
  return false;
}
...
function validateCreditCardForm() {
  if (
   validateCreditCard(document.querySelector('#welight_credit_card_number')) &&
...
function validateCreateToken() {
  if(validateCreditCard(document.querySelector('#welight_credit_card_number')) 
    && validateCreditCardMonth(document.querySelector('#creditCardExpirationMonth'))
    && validateCreditCardYear(document.querySelector('#creditCardExpirationYear'))
    && validateCreditCardCode(document.querySelector('#creditCardCode'))
