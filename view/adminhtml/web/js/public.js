...
var WS = {
    ...
    'Ajax' : {
        ...
        'Conciliation' : {
            ...
            {
                jQuery.ajax( {
                    url: url + '/welight/conciliation/request',
                    ...
                    if (response.success) {
                        ...
                        var t = jQuery('#welight-datatable').DataTable();
            ...
            {
                var t    = jQuery('#welight-datatable').DataTable();
                var rows = jQuery('#welight-datatable').find('[data-target=conciliation]:checked');
                ...
                jQuery.ajax( {
                    url: url + '/welight/conciliation/conciliate',
        ...
        'Abandoned' : {
            ...
            'Search': function (url) {
                ...
                jQuery.ajax({
                    url: url + '/welight/abandoned/request',
            ...
            {
                var t    = jQuery('#welight-datatable').DataTable();
                var rows = jQuery('#welight-datatable').find('[data-target=abandoned]:checked');
                ...
                jQuery.ajax( {
                    url: url + '/welight/abandoned/transport',
                    data: {form_key: window.FORM_KEY, data: data},
                    type: 'POST',
                    showLoader: true,
                }).success(function(response) {
        ...
        'Cancellation' : {
            ...
            {
                jQuery.ajax( {
                    url: url + '/welight/cancellation/request',
            ...
            'Cancel' : function(url, data, row)
            {
                ...
                jQuery.ajax( {
                    url: url + '/welight/cancellation/cancel',
                    data: {form_key: window.FORM_KEY, data: data},
                    type: 'POST',
                    showLoader: true,
                }).success(function(response) {
        ...
        'Refund' : {
            ...
            {
                jQuery.ajax( {
                    url: url + '/welight/refund/request',
                ...
                jQuery.ajax( {
                    ...
                    if (response.success) {
                        ...
                        if (response.payload.error == 'Need to conciliate') {
                            ...
                            Modal.Load('Estorno', "Sua conta welight não tem permissão para realizar esta ação. Em caso de dúvidas acesse <a href='http://forum.welight.uol.com.br' target='_blank'>http://forum.welight.uol.com.br</a>");
                        }else {
                            Modal.Load('Estorno', 'Não foi possível executar esta ação. Tente novamente mais tarde.');
                        }
                    }
        ...
        'Transactions' : {
                        ...
                        if (response.payload.data.length > 0) {
                            ...
                            response.payload.data.forEach(function(item){
                                t.row.add( [
                                    ...
                                    item.environment,
                                    item.magento_status,
                                    '<a class="link" data-transaction="'+ item.welight_id +'" data-order="'+ item.order_id +'">Ver detalhes do pagamento</a>'
                                ] );
            ...
            'Details' : function (url, transaction_code, order) {
                jQuery.ajax({
                    url: url + '/welight/transactions/transaction',
                    type: "POST",
                    data: {
                        form_key: window.FORM_KEY,
